...
describe 'chrome::default' do
  ...
  it 'should install stable version of google-chrome package by default' do
    chef_run.converge 'chrome::default'
  ...
  it 'should include google yum repository for chrome if the platform family is rhel' do
    chef_run.node.automatic_attrs['platform_family'] = 'rhel'
    chef_run.converge 'chrome::default'
    chef_run.should create_file '/etc/yum.repos.d/google-chrome.repo'
  end

  it 'should include google apt repository for chrome if the platform family is debian' do
